...
export default function SecretNotes({ onClose }) {
  ...
  const handlePasswordSubmit = (password: string) => {
    if (passwordModalMode === 'set') {
      ...
      if (note && note.password === password) {
        setNotes(notes.map((note) => 
          note.id === activeNoteId ? { ...note, locked: false } : note
        ))
      } else {
  ...
  return (
    <div className="secret-notes p-4 position-relative">
      ...
      <div className="mb-4">
        {sortedNotes.some(note => note.favorite && !note.archived) && (
          <div className="mb-4">
...
function NoteItem({ note, onSelect, onToggleFavorite, onToggleArchive, onToggleLock }) {
  return (
    <motion.li
      ...
      whileTap={{ scale: 0.98 }}
    >
      <div className="flex-grow-1 me-3" onClick={() => !note.locked && onSelect(note)}>
